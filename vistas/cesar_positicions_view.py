...
class CaesarPositionsWorkspaceView(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        self.entry_text.insert(0, "MENSACERO")
        self.entry_text.grid(row=0, column=1, padx=8, pady=10)

        ctk.CTkLabel(ctrl_frame, text="Desplazamiento:").grid(row=0, column=2, padx=8, pady=10)
        self.entry_shift = ctk.CTkEntry(ctrl_frame, width=70)
        self.entry_shift.insert(0, "3")
        self.entry_shift.grid(row=0, column=3, padx=8, pady=10)

        self.switch_mode = ctk.CTkSwitch(ctrl_frame, text="Modo Descifrado")
        ...
        self.lbl_explanation = ctk.CTkLabel(
            res_frame, text="Ingresa el texto y el valor de desplazamiento, luego presiona 'Iniciar Animación'.", 
    ...
    def run_animation(self):
        ...
        is_descifrado = bool(self.switch_mode.get())

        steps = self.cipher.encrypt_steps(text, shift_str, descifrado=is_descifrado)
        if not steps:
            return
    ...
    def _execute_step(self, steps, current_index):
        if current_index < len(steps):
            ...
            if step['p_idx'] is not None:
                ...
                shift_symbol = "➔" if step['shift'] >= 0 else "⬅"
                ctk.CTkLabel(box_frame, text=f"{shift_symbol} ({step['shift']})", font=ctk.CTkFont(size=16, weight="bold")).grid(row=0, column=1, padx=10)

                # Posición Destino
                box_c = ctk.CTkFrame(box_frame, fg_color="#10b981", corner_radius=6)
                box_c.grid(row=0, column=2, padx=8)
